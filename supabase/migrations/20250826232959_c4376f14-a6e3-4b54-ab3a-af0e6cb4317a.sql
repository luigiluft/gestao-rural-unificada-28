-- Corrigir função complete_pallet_allocation_and_create_stock
-- Remover todas as referências a posicao_id na criação de estoque
...
BEGIN
    ...
    -- IMPORTANTE: Estoque SEM posicao_id - consolidado por user/produto/deposito/lote
    FOR produto_conf IN 
        SELECT 
    ...
    LOOP
        -- Check if there's already an estoque entry (SEM posicao_id)
        ...
        IF v_existing_estoque_id IS NOT NULL THEN
            -- Update existing stock (consolidado)
        ...
        ELSE
            -- Create new stock entry (SEM posicao_id - estoque consolidado)
