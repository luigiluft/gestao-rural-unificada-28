-- Fix the main allocation function to remove posicao_id references from estoque table
CREATE OR REPLACE FUNCTION public.complete_pallet_allocation_and_create_stock(p_wave_pallet_id uuid, p_posicao_id uuid, p_barcode_pallet text, p_barcode_posicao text, p_produtos_conferidos jsonb, p_divergencias jsonb DEFAULT '[]'::jsonb)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
    ...
    LOOP
        -- Check if there's already an estoque entry for this combination (without posicao_id)
        ...
        ELSE
            -- Create new stock entry (without posicao_id)
...
DROP FUNCTION IF EXISTS public.complete_pallet_allocation_and_create_stock_debug(uuid, uuid, text, text, jsonb, jsonb);