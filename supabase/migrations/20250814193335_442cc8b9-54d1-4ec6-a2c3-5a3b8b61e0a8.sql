-- Migração: Mover roles para tabela profiles
...
-- Fase 6: Remover trigger e função relacionados ao user_roles
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF NEW.role = 'produtor' AND OLD.role != 'produtor' THEN
      INSERT INTO public.produtores(user_id) VALUES (NEW.user_id)
      ON CONFLICT (user_id) DO NOTHING;
...
-- Fase 8: Atualizar RLS policies que usavam user_roles
-- Atualizar policies para usar profiles ao invés de user_roles
...
-- Adicionar policy para permitir usuários visualizarem seu próprio role
CREATE POLICY "Users can view their own role" ON public.profiles
FOR SELECT USING (auth.uid() = user_id);

-- Permitir usuários atualizarem seus próprios dados (exceto role)
