...
-- Verificar tipo atual da coluna status e alterar se necessário  
DO $$ 
DECLARE
    current_type text;
BEGIN
    ...
    -- Se a coluna é text, alterar para enum
    IF current_type = 'text' THEN
        -- Remover default temporariamente
        ALTER TABLE public.pallet_positions ALTER COLUMN status DROP DEFAULT;
        
        -- Alterar tipo usando casting explícito de text para enum
        ALTER TABLE public.pallet_positions 
        ALTER COLUMN status TYPE pallet_status 
        USING CASE 
            WHEN status = 'alocado' THEN 'alocado'::pallet_status
            WHEN status = 'removido' THEN 'removido'::pallet_status
            ELSE 'pendente'::pallet_status
        END;
