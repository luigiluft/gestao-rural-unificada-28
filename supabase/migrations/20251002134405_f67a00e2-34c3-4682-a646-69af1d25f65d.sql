-- Drop and recreate get_estoque_from_movimentacoes with proper avaria filtering
...
BEGIN
  ...
  WITH movimentacoes_consolidadas AS (
    ...
    WHERE pp.status = 'alocado'
      AND epi.is_avaria = false  -- FILTRO CRÍTICO: apenas itens não avariados
    GROUP BY 
      ei.produto_id, 
      e.deposito_id, 
      e.user_id, 
    ...
    -- Movimentações de allocation_wave (não tem avarias)
    SELECT 
      m.produto_id,
      m.deposito_id,
      m.user_id,
    ...
    -- Movimentações de saída (todas as saídas reduzem estoque)
    SELECT 
      m.produto_id,
      m.deposito_id,
      m.user_id,
