-- Migração: Mover roles para tabela profiles (Correção de Trigger)
...
-- Fase 6: Remover TODOS os triggers relacionados aos user_roles
...
DROP TRIGGER IF EXISTS trg_sync_produtor_on_role_ins ON public.user_roles;
DROP TRIGGER IF EXISTS trg_sync_produtor_on_role_del ON public.user_roles;

-- Agora remover as funções com CASCADE para garantir
DROP FUNCTION IF EXISTS public.validate_produtor_has_franqueado() CASCADE;
DROP FUNCTION IF EXISTS public.sync_produtor_row_on_role() CASCADE;
...
-- Corrigindo o trigger para profiles
...
-- Fase 8: Limpar tabela user_roles obsoleta
