...
SELECT 
    ...
    CASE 
        ...
        ELSE 0
    END as valor_medio,
    -- Data de validade mais recente para o lote
    (SELECT data_validade 
     FROM movimentacoes m2 
     WHERE m2.user_id = m.user_id 
       AND m2.produto_id = m.produto_id 
       AND m2.deposito_id = m.deposito_id 
       AND COALESCE(m2.lote, '') = COALESCE(m.lote, '')
       AND m2.data_validade IS NOT NULL
     ORDER BY m2.data_movimentacao DESC 
