...
-- 2. Criar view materializada baseada exclusivamente em movimentacoes (sem RLS)
CREATE MATERIALIZED VIEW public.estoque_base AS
...
-- 3. Criar view regular com RLS em cima da materializada
CREATE VIEW public.estoque AS
SELECT * FROM public.estoque_base;

-- 4. Criar índices para performance na view materializada
CREATE INDEX idx_estoque_base_user_produto_deposito ON public.estoque_base(user_id, produto_id, deposito_id);
CREATE INDEX idx_estoque_base_user_produto ON public.estoque_base(user_id, produto_id);
CREATE INDEX idx_estoque_base_deposito ON public.estoque_base(deposito_id);
CREATE INDEX idx_estoque_base_quantidade ON public.estoque_base(quantidade_atual) WHERE quantidade_atual > 0;

-- 5. RLS para a view regular
...
-- 6. Nova função get_estoque_seguro baseada na view materializada
...
-- 7. Função para refresh da view materializada
...
BEGIN
    ...
    WHILE current_attempt < max_attempts AND NOT success LOOP
        ...
        BEGIN
            REFRESH MATERIALIZED VIEW estoque_base;
            success := TRUE;
...
-- 8. Triggers para refresh automático da view
...
-- 9. Popular a view materializada com dados iniciais
REFRESH MATERIALIZED VIEW public.estoque_base;

-- 10. Permissões
GRANT SELECT ON public.estoque TO authenticated;
GRANT SELECT ON public.estoque_base TO authenticated;
...
COMMENT ON MATERIALIZED VIEW public.estoque_base IS 'View materializada que calcula estoque exclusivamente baseado no ledger de movimentações';
COMMENT ON VIEW public.estoque IS 'View com RLS que expõe dados da view materializada estoque_base respeitando permissões por usuário';
