-- Reorganizar sistema de roles para eliminar confusão
...
-- 6. Adicionar constraint para validar hierarquia de roles
-- Admin pode criar templates para todos os roles
-- Franqueado pode criar templates apenas para franqueado e produtor
-- Produtor pode criar templates apenas para produtor
ALTER TABLE public.permission_templates 
ADD CONSTRAINT validate_role_hierarchy 
CHECK (
  (target_role = 'admin' AND EXISTS (SELECT 1 FROM profiles WHERE user_id = permission_templates.user_id AND role = 'admin')) OR
  (target_role = 'franqueado' AND EXISTS (SELECT 1 FROM profiles WHERE user_id = permission_templates.user_id AND role IN ('admin', 'franqueado'))) OR
  (target_role = 'produtor' AND EXISTS (SELECT 1 FROM profiles WHERE user_id = permission_templates.user_id AND role IN ('admin', 'franqueado', 'produtor')))
);

-- 7. Adicionar comentários para documentar a hierarquia
...
-- 8. Atualizar RLS policies para refletir nova nomenclatura
...
-- 9. Atualizar policies para user_permission_templates
...
ON public.user_permission_templates FOR SELECT
USING (user_id = auth.uid());