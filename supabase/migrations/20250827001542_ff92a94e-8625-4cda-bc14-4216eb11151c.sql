-- Rewrite complete_pallet_allocation_and_create_stock function to use atomic INSERT ON CONFLICT
...
BEGIN
    ...
    -- Process each confirmed product to create stock entries using atomic INSERT ON CONFLICT
    FOR produto_conf IN 
        SELECT 
    ...
    LOOP
        -- Use atomic INSERT ON CONFLICT DO UPDATE to handle concurrent operations
        ...
        DO UPDATE SET 
            quantidade_atual = estoque.quantidade_atual + produto_conf.quantidade_conferida,
            valor_medio = CASE 
                WHEN estoque.quantidade_atual > 0 THEN 
                    ((estoque.valor_medio * estoque.quantidade_atual) + (produto_conf.valor_unitario * produto_conf.quantidade_conferida)) / (estoque.quantidade_atual + produto_conf.quantidade_conferida)
                ELSE 
                    produto_conf.valor_unitario
            END,
            updated_at = now();
