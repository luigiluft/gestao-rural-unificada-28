-- Remove any old versions of the function and recreate the correct one
DROP FUNCTION IF EXISTS public.complete_pallet_allocation_and_create_stock1;

-- Ensure the correct function exists without any ON CONFLICT issues
...
BEGIN
    ...
    LOOP
        ...
        ELSE
            -- Create new stock entry (manual check prevents duplicates)
        ...
        -- Create movement record for each product
        INSERT INTO public.movimentacoes (
            ...
            'allocation_wave',
            produto_conf.lote,
            'Alocação de pallet via onda: ' || v_pallet.numero_pallet,
            now()
        );
    END LOOP;

    -- Check if all pallets in the wave are allocated to complete the wave
    SELECT COUNT(*)
    INTO v_pending_pallets_count
    FROM public.allocation_wave_pallets
    WHERE wave_id = v_wave_id 
      AND status IN ('pendente', 'posicao_definida');

    -- If no more pending pallets, mark wave as completed
