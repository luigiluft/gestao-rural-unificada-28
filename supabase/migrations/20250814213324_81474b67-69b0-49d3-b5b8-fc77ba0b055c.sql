-- Create function to handle entry products and update stock
...
BEGIN
    ...
    IF NEW.produto_id IS NULL THEN
        -- For now, we'll create a basic product entry with the lote as name
        -- In a real scenario, this would be handled by the NFe parser
...
-- Process existing entrada_itens that haven't been processed
DO $$
DECLARE
    item RECORD;
BEGIN
    FOR item IN 
        ...
        WHERE created_at >= '2025-01-01' -- Only recent entries
        ORDER BY created_at ASC
    LOOP
        -- Trigger the function manually for existing items
        PERFORM public.process_entrada_item() FROM (SELECT item.*) AS NEW;
    END LOOP;
END $$;