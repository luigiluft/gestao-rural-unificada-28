...
-- Limpar os registros órfãos da tabela movimentacoes
DELETE FROM public.movimentacoes 
WHERE referencia_tipo IN ('allocation_wave', 'allocation_wave_pallet');

-- Adicionar constraint para validar referencia_tipo incluindo todos os tipos válidos encontrados
ALTER TABLE public.movimentacoes 
ADD CONSTRAINT valid_referencia_tipo 
CHECK (referencia_tipo IN ('entrada', 'saida', 'pallet', 'inventario', 'ajuste', 'separacao'));

-- Atualizar view materializada do estoque após limpeza (sem CONCURRENTLY)
