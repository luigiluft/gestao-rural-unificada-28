-- 1. Função para verificar se um produto tem posição física alocada
...
-- 2. Função para alocar automaticamente produtos órfãos em posições disponíveis
...
-- 3. Trigger para validar integridade antes de criar/atualizar estoque
...
-- 4. Aplicar trigger na view materializada estoque
-- Como não podemos colocar trigger direto na view, vamos criar na tabela movimentacoes
...
-- Criar triggers
DROP TRIGGER IF EXISTS trigger_validar_movimentacao_entrada ON movimentacoes;
CREATE TRIGGER trigger_validar_movimentacao_entrada
  BEFORE INSERT OR UPDATE ON movimentacoes
  FOR EACH ROW
  EXECUTE FUNCTION validar_movimentacao_entrada();

-- 5. Executar alocação automática para produtos órfãos existentes
SELECT public.alocar_produtos_orfaos() as produtos_alocados;