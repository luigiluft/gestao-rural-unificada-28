...
reservas AS (
    -- Calcular quantidade reservada por produto/deposito
    SELECT 
        si.produto_id,
        s.deposito_id,
    ...
    WHERE s.status IN ('aprovado', 'em_separacao')
...
CREATE UNIQUE INDEX idx_estoque_unique_ledger ON public.estoque(user_id, produto_id, deposito_id, md5(COALESCE(array_to_string(lotes, ','), '')));
