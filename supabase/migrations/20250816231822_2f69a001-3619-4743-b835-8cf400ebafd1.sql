...
ALTER TABLE saidas ALTER COLUMN status DROP DEFAULT;

-- Update existing data to match enum values
UPDATE saidas SET status = 'separacao_pendente' WHERE status = 'processando' OR status = 'preparando' OR status IS NULL;
UPDATE saidas SET status = 'expedido' WHERE status = 'expedida';
UPDATE saidas SET status = 'entregue' WHERE status = 'entregue';

-- Now convert the column type
ALTER TABLE saidas ALTER COLUMN status TYPE saida_status USING 
  CASE 
    WHEN status = 'processando' OR status = 'preparando' THEN 'separacao_pendente'::saida_status
    WHEN status = 'expedida' THEN 'expedido'::saida_status
    WHEN status = 'entregue' THEN 'entregue'::saida_status
    ELSE 'separacao_pendente'::saida_status
  END;
