-- Remove the old constraint that doesn't consider position
ALTER TABLE public.estoque DROP CONSTRAINT IF EXISTS estoque_produto_id_deposito_id_lote_key;

-- Add new constraint that includes position for better organization
-- Multiple positions can have the same product+lote in the same deposit
-- But each position should have unique product+lote combinations
ALTER TABLE public.estoque ADD CONSTRAINT estoque_produto_deposito_posicao_lote_unique 
UNIQUE (produto_id, deposito_id, posicao_id, lote);

-- Update the RPC function to handle position-based stock correctly
...
BEGIN
    ...
    LOOP
        ...
        INSERT INTO public.movimentacoes (
            ...
            'allocation_wave',
            produto_conf.lote,
            'Entrada via alocação - Pallet: ' || p_barcode_pallet || ' - Posição: ' || p_barcode_posicao,
            now()
        );
    END LOOP;
