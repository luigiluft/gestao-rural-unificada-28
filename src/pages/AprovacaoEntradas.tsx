...
export default function AprovacaoEntradas() {
  ...
  const updateDivergencia = async (index: number, field: keyof Divergencia, value: any) => {
    ...
    if (field === 'produto_id' && value) {
      try {
        const {
          data: produto
        } = await supabase.from('produtos').select('containers_per_package, package_capacity').eq('id', value).single();
        if (produto) {
  ...
  const calcularIncremento = async (produtoId: string) => {
    ...
    try {
      const {
        data: produto
      } = await supabase.from('produtos').select('containers_per_package, package_capacity').eq('id', produtoId).maybeSingle();
      if (produto) {
  ...
  const processarLeituraBarra = () => {
    ...
    const item = selectedEntrada.entrada_itens?.find((item: any) => item.produtos?.codigo === codigoBarras.trim() || item.nome_produto?.toLowerCase().includes(codigoBarras.trim().toLowerCase()));
    if (!item) {
      ...
      if (leituraExistente) {
        const novasLeituras = leituras.map(l => l.codigo === codigoBarras.trim() ? {
          ...l,
          quantidade: l.quantidade + quantidadeBip,
          timestamp: new Date()
        } : l);
  ...
  const diminuirQuantidade = (codigo: string) => {
    setLeituras(leituras.map(l => l.codigo === codigo ? {
      ...l,
      quantidade: Math.max(0, l.quantidade - 1)
    } : l).filter(l => l.quantidade > 0));
  };
  ...
  const gerarDivergenciasAutomaticas = async () => {
    ...
    for (const itemEsperado of selectedEntrada.entrada_itens) {
      ...
      const leituraCorrespondente = leituras.find(l => l.codigo === produto_codigo || l.produto_nome === produto_nome);
      ...
      if (quantidade_esperada !== quantidade_recebida) {
        ...
        divergenciasCalculadas.push({
          ...
          quantidade_recebida,
          quantidade_avariada: 0,
          motivo: quantidade_recebida < quantidade_esperada ? `Quantidade inferior ao esperado (falta: ${quantidade_esperada - quantidade_recebida})` : `Quantidade superior ao esperado (excesso: ${quantidade_recebida - quantidade_esperada})`,
    ...
    leituras.forEach(leitura => {
      const itemEsperado = selectedEntrada.entrada_itens?.find((item: any) => item.produtos?.codigo === leitura.codigo || (item.produtos?.nome || item.nome_produto) === leitura.produto_nome);
      ...
      <Tabs defaultValue="aguardando_transporte" className="space-y-4">
        ...
        return <TabsContent key={status} value={status} className="space-y-4">
              {statusEntradas.length === 0 ? <EmptyState title="Nenhum pedido de recebimento" description={getEmptyStateDescription(status)} /> : <div className="grid gap-4">
                {(statusEntradas as any[]).map(entrada => <Card key={entrada.id}>
                    <CardHeader>
                        ...
                        <div className="space-y-1">
                         ...
                         <div className="flex gap-2">
                           {entrada.status_aprovacao === 'aguardando_conferencia' && <>
                               <Button onClick={() => handleAction(entrada, 'conferencia_barras')} size="sm" variant="outline" disabled={atualizarStatus.isPending}>
                                 <Scan className="h-4 w-4 mr-2" />
                                 Conferência por Código de Barras
                               </Button>
                               <Button onClick={() => handleAction(entrada, 'conferencia')} size="sm" disabled={atualizarStatus.isPending}>
                    ...
                    <CardContent>
                      ...
                      {entrada.entrada_itens?.length > 0 && <div className="mt-4">
                          ...
                          <Table>
                            ...
                            <TableBody>
                              {entrada.entrada_itens.map((item: any) => <TableRow key={item.id}>
                      ...
                      {entrada.divergencias && entrada.divergencias.length > 0 && <div className="mt-4">
                          <Separator className="mb-4" />
                    ...
                    return <div key={index} className="p-3 border rounded-lg bg-destructive/5">
                                <div className="text-sm space-y-1">
                                  ...
                                  {div.tipo_divergencia === 'quantidade' ? <div><strong>Diferença:</strong> {diferencaFormatada}</div> : <div><strong>Quantidade Avariada:</strong> {div.quantidade_avariada || 0}</div>}
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogHeader>
            <DialogTitle>
              {actionType === 'conferencia_barras' ? 'Conferência por Código de Barras' : actionType === 'conferencia' ? 'Conferência Manual' : 'Atualizar Status'}
            </DialogTitle>
            <DialogDescription>
              {actionType === 'conferencia_barras' ? 'Use o coletor de código de barras para realizar a conferência automaticamente' : actionType === 'conferencia' ? 'Confira os itens recebidos e registre eventuais divergências manualmente' : `Confirmar mudança de status para "${getNextStatusLabel(selectedEntrada?.status_aprovacao || '')}"`}
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4">
            {actionType === 'conferencia_barras' && <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    <div className="col-span-2">
                      ...
                      <Input id="codigo-barras" ref={inputBarrasRef} value={codigoBarras} onChange={e => setCodigoBarras(e.target.value)} onKeyPress={handleKeyPress} placeholder="Posicione o cursor aqui e use o coletor" className="font-mono text-lg" autoFocus />
                    </div>
                    <div>
                      <Label htmlFor="quantidade">Quantidade</Label>
                      <Input id="quantidade" type="number" min="1" value={quantidadeBip} onChange={e => setQuantidadeBip(Math.max(1, parseInt(e.target.value) || 1))} className="text-center" />
                    </div>
                    <div className="flex items-end">
                      <Button onClick={processarLeituraBarra} disabled={!codigoBarras.trim()} className="w-full" variant="outline">
                        <Scan className="h-4 w-4 mr-2" />
                        Adicionar
                      </Button>
                    </div>
                ...
                {leituras.length > 0 && <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <Label className="text-base font-semibold">Leituras Realizadas ({leituras.length})</Label>
                       <Button onClick={async () => await gerarDivergenciasAutomaticas()} size="sm" variant="outline">
                    ...
                    <div className="border rounded-lg max-h-60 overflow-y-auto">
                      <Table>
                        ...
                        <TableBody>
                          {leituras.map((leitura, index) => <TableRow key={index}>
                              <TableCell className="font-mono">{leitura.codigo}</TableCell>
                              <TableCell>{leitura.produto_nome}</TableCell>
                              <TableCell>
                                <div className="flex items-center gap-1">
                                  <Button onClick={() => diminuirQuantidade(leitura.codigo)} size="sm" variant="outline" className="h-6 w-6 p-0">
                                    -
                                  </Button>
                                  <span className="mx-2 font-medium">{leitura.quantidade}</span>
                                  <Button onClick={() => aumentarQuantidade(leitura.codigo)} size="sm" variant="outline" className="h-6 w-6 p-0">
                                    +
                                  </Button>
                                </div>
                               </TableCell>
                              ...
                              <TableCell>
                                <Button onClick={() => removerLeitura(leitura.codigo)} size="sm" variant="outline" className="text-destructive hover:text-destructive">
                                  <X className="h-4 w-4" />
                                </Button>
                              </TableCell>
                            </TableRow>)}
                ...
                {divergencias.length > 0 && <div className="space-y-2">
                    <Label className="text-base font-semibold text-destructive">
                      Divergências Detectadas ({divergencias.length})
                    </Label>
                    <div className="space-y-2">
                      {divergencias.map((div, index) => <Card key={index} className="bg-destructive/5">
            ...
            {actionType === 'conferencia' && <div className="space-y-4">
                {/* Lista de itens para seleção rápida */}
                {selectedEntrada?.entrada_itens?.length > 0 ? <div className="space-y-2">
                    ...
                    <div className="border rounded-lg p-4 bg-muted/30">
                      <Table>
                        ...
                        <TableBody>
                           {selectedEntrada.entrada_itens.map((item: any) => <TableRow key={item.id}>
                               <TableCell>{item.produtos?.nome || item.nome_produto}</TableCell>
                               <TableCell>{item.quantidade} {item.produtos?.unidade_medida}</TableCell>
                                <TableCell>{item.lote || '-'}</TableCell>
                                <TableCell>
                                   <Button onClick={async () => await addDivergencia(item)} size="sm" variant="outline" className="text-xs">
                   ...
                   </div> : <div className="border rounded-lg p-4 bg-muted/30">
                     <div className="text-center text-muted-foreground">
                       ...
                       {selectedEntrada && <div className="mt-2 text-xs">
                ...
                <div className="flex items-center">
                  <Label className="text-base font-semibold">Divergências</Label>
                   <Button onClick={async () => await addDivergencia()} size="sm" variant="outline" className="ml-auto">
                    <Plus className="h-4 w-4 mr-2" />
                    Adicionar Divergência
                  </Button>
                </div>

                {divergencias.map((div, index) => <Card key={index}>
                    <CardContent className="p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <Label className="font-medium">Divergência {index + 1}</Label>
                        <Button onClick={() => removeDivergencia(index)} size="sm" variant="outline" className="text-destructive hover:text-destructive">
                       ...
                       <div className="space-y-3">
                         <div className="grid md:grid-cols-2 gap-3">
                           <div>
                             <Label>Produto</Label>
                             <Input value={div.produto} onChange={e => updateDivergencia(index, 'produto', e.target.value)} placeholder="Nome do produto" />
                           </div>
                           <div>
                             <Label>Lote</Label>
                             <Input value={div.lote} onChange={e => updateDivergencia(index, 'lote', e.target.value)} placeholder="Número do lote" />
                           </div>
                         </div>
                         ...
                         <div className="grid md:grid-cols-3 gap-3">
                           <div>
                             ...
                             <Select value={div.tipo_divergencia} onValueChange={value => updateDivergencia(index, 'tipo_divergencia', value)}>
                               <SelectTrigger>
                           ...
                           {div.tipo_divergencia === 'quantidade' && <>
                               <div>
                                 ...
                                 <Input type="number" value={div.quantidade_esperada} readOnly className="bg-muted cursor-not-allowed" title="Quantidade do XML - não pode ser alterada" />
                               </div>
                               <div>
                                 <Label>Qtd. Recebida</Label>
                                 <Input type="number" value={div.quantidade_recebida} onChange={e => {
                          const value = parseFloat(e.target.value) || 0;
                          updateDivergencia(index, 'quantidade_recebida', value);
                        }} step={div.incremento || 1} min="0" />
                               </div>
                             </>}
                           
                           {div.tipo_divergencia === 'avaria' && <div className="md:col-span-2">
                               <Label>Qtd. Avariada</Label>
                               <Input type="number" value={div.quantidade_avariada || 0} onChange={e => {
                        const value = parseFloat(e.target.value) || 0;
                        updateDivergencia(index, 'quantidade_avariada', value);
                      }} step={div.incremento || 1} min="0" max={div.quantidade_esperada} />
                             </div>}
                         </div>
                       </div>
                      ...
                      <div>
                        ...
                        <Textarea value={div.motivo} onChange={e => updateDivergencia(index, 'motivo', e.target.value)} placeholder="Descreva o motivo da divergência" />
                      </div>
                      
                      <div>
                        <Label>Ação Tomada</Label>
                        <Textarea value={div.acao_tomada} onChange={e => updateDivergencia(index, 'acao_tomada', e.target.value)} placeholder="Descreva a ação tomada" />
                      </div>
                    </CardContent>
                  </Card>)}
              </div>}
            ...
            <div>
              ...
              <Textarea value={observacoes} onChange={e => setObservacoes(e.target.value)} placeholder="Adicione observações sobre esta etapa (opcional)" rows={3} />
            </div>
          </div>
