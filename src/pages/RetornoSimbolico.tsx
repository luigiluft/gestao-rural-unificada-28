...
export default function RetornoSimbolico() {
  ...
  // Filtrar apenas retornos simbólicos
  const entradasFiltradas = entradas?.filter(e => 
    e.natureza_operacao?.toLowerCase().includes('retorno') && 
    e.natureza_operacao?.toLowerCase().includes('simbólico')
  ) || [];
  ...
  const handleExport = () => {
    ...
    exportToCSV(csvData, 'retorno-simbolico');
  };
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Retorno Simbólico</h1>
          <p className="text-muted-foreground mt-2">
            Notas fiscais de retorno simbólico de mercadorias
          </p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <EmptyState
              title="Nenhum retorno simbólico encontrado"
              description="Não há notas fiscais de retorno simbólico no período selecionado."
            />
          ) : (
