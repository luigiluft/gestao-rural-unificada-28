...
export default function AlocacaoPallets() {
  ...
  const processNextWavePallet = async () => {
    ...
    const remainingPallets = selectedPallets.filter(palletId => !processedPallets.has(palletId) && palletsPendentes?.find(p => p.id === palletId));
  ...
  const confirmWaveStep = async () => {
    ...
    try {
      ...
      const remainingUnprocessed = selectedPallets.filter(id => !processedPallets.has(id) && id !== currentResult.pallet.id);
  ...
  return <div className="space-y-6 p-6">
      ...
      {!palletsPendentes?.length ? <EmptyState icon={<Package className="h-8 w-8" />} title="Nenhum pallet pendente" description="Todos os pallets foram alocados ou não há entradas aprovadas." /> : <div className="space-y-4">
          ...
          <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg border">
            <div className="flex items-center gap-4">
              ...
              {selectedPallets.length > 0 && <Button size="sm" variant="outline" onClick={() => setSelectedPallets([])}>
                  Limpar Seleção
                </Button>}
            </div>

            <div className="flex gap-2 flex-wrap sm:flex-nowrap">
              <Button size="sm" variant="outline" disabled={selectedPallets.length === 0 || isWaveMode !== null || isProcessing} onClick={handleWaveManualAllocation} className="flex-shrink-0">
                <Hand className="h-4 w-4 mr-2" />
                {isProcessing ? "Processando..." : "Alocar Manualmente"}
              </Button>
              <Button size="sm" disabled={selectedPallets.length === 0 || isWaveMode !== null || isProcessing} onClick={handleWaveScannerAllocation} className="flex-shrink-0">
          ...
          {/* Progresso da onda */}
          {waveProgress && <div className="p-4 bg-primary/10 border border-primary/20 rounded-lg">
            ...
            width: `${waveProgress.completed / waveProgress.total * 100}%`
          }} />
              </div>
            </div>}

          {/* Confirmação da onda atual */}
          {isWaveMode && waveResults.length > 0 && waveProgress && waveProgress.completed < waveProgress.total && <div className="p-4 bg-muted rounded-lg border">
              {(() => {
          const currentResult = waveResults[waveResults.length - 1];
          return <div>
                    ...
                    <div className="mb-3">
                      ...
                      <div className="flex gap-2">
                        <Input id="wavePositionInput" value={isEditingPosition ? customPosition : currentResult.result.posicao_codigo} onChange={e => setCustomPosition(e.target.value)} readOnly={!isEditingPosition} className="text-sm" />
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" onClick={confirmWaveStep} disabled={confirmAllocation.isPending}>
                        {confirmAllocation.isPending ? "Confirmando..." : "Confirmar Posicionamento"}
                      </Button>
                      <Button size="sm" variant="outline" onClick={resetWaveState}>
                        Cancelar Onda
                      </Button>
                    </div>
          ...
          <div className="border rounded-lg">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-12">
                    <Checkbox checked={selectedPallets.length === palletsPendentes.length} onCheckedChange={handleSelectAll} disabled={isWaveMode !== null} />
                  </TableHead>
              ...
              <TableBody>
                {palletsPendentes.map(pallet => <TableRow key={pallet.id} className="hover:bg-muted/50">
                    <TableCell>
                      <Checkbox checked={selectedPallets.includes(pallet.id)} onCheckedChange={checked => handleSelectPallet(pallet.id, !!checked)} disabled={isWaveMode !== null} />
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Package className="h-4 w-4 text-muted-foreground" />
                        <span className="font-medium">#{pallet.numero_pallet}</span>
                        {pallet.descricao && <Badge variant="outline" className="text-xs">
                            {pallet.descricao}
                          </Badge>}
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      {format(new Date(pallet.created_at), "dd/MM/yyyy", {
                  locale: ptBR
                })}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        {pallet.entrada_pallet_itens?.length > 0 && <div className="text-xs text-muted-foreground">
                            {pallet.entrada_pallet_itens.slice(0, 2).map((item, i) => <div key={i}>
                                {item.entrada_itens?.nome_produto?.substring(0, 20)}...
                              </div>)}
                            {pallet.entrada_pallet_itens.length > 2 && <div>+{pallet.entrada_pallet_itens.length - 2} mais</div>}
                          </div>}
                      </div>
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="flex gap-1 justify-end">
                        <Button size="sm" variant="outline" disabled={autoAllocatePallet.isPending || activeMethod === "manual" || isWaveMode !== null} onClick={() => handleStartManualAllocation(pallet)}>
                          <Hand className="h-3 w-3" />
                        </Button>
                        <Button size="sm" disabled={autoAllocatePallet.isPending || activeMethod === "scanner" || isWaveMode !== null} onClick={() => handleStartScannerAllocation(pallet)}>
          ...
          {!isWaveMode && activeMethod && activePallet && allocationResult && <div className="p-4 bg-muted rounded-lg border">
              ...
              {activeMethod === "manual" ? <div>
                  ...
                  <div className="mb-3">
                    ...
                    <div className="flex gap-2">
                      <Input id="individualPositionInput" value={isEditingPosition ? customPosition : allocationResult.posicao_codigo} onChange={e => setCustomPosition(e.target.value)} readOnly={!isEditingPosition} className="text-sm" />
                      <Button size="sm" variant="outline" onClick={() => {
                if (isEditingPosition) {
                  setCustomPosition("");
                } else {
                ...
                setIsEditingPosition(!isEditingPosition);
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" onClick={handleManualConfirmation} disabled={confirmAllocation.isPending}>
                      {confirmAllocation.isPending ? "Confirmando..." : "Confirmar que coloquei o pallet"}
                    </Button>
                    <Button size="sm" variant="outline" onClick={resetAllocationState}>
                      Cancelar
                    </Button>
                  </div>
                </div> : <div className="space-y-3">
